...
class LoadProfileError(Exception):
    ...
    def __init__(self, mode):
        ...
        Args:
          self: passes exception
          mode: error type,
            0 if the time stamps are not matching between the active load profile,
                reactive load profile, and EV charging profile.
...
class LVGrid:
    ...
    def __init__(
        ...
        """
        Validity checks need to be made to so that there is no overlap or mismatching between the
        relevant IDs and profiles. Read 'input validity check' for the specific checks.
        also raise or passthrough relevant errors.
        
        NOTE: the funtionalities are independent from each other. For example, 
            for optimal tap position analysis, 
            you need to analyse the original grid with house profile, WITHOUT the EV profile.
        """

    def optimal_tap_position(self, optimization_criteri: str):
        """
        Optimize the tap position of the transformer in the LV grid.
        Run a one time power flow calculation on every possible tap postition for every timestamp
        (https://power-grid-model.readthedocs.io/en/stable/examples/Transformer%20Examples.html).
        The most opmtimized tap position should have the min total energy loss of all lines
            and whole period and min. deviation of p.u. node voltages w.r.t. 1 p.u.
        (We think that total energy loss has more importance than the Delta p.u.)
        The user can choose the criteria for optimization, so thye can choose how low the
            energy_loss and voltage_deviation should be for it to be valid.

        Args:
            optimization_criteria (str): Criteria for optimization
            (e.g., 'energy_loss', 'voltage_deviation').
    ...
    def ev_penetration_level(self, penetration_level: float):
        """
        Randomly adding EV charging profiles according to a couple criterea
            using the input 'penetration_level'.

        First: The number of EVs per LV feeder needs to be equal to
            'round_down[penetration_level * total_houses / number_of_feeders]'
        ...
        Then within each feeder randomly select houses whih will have an EV charger.
        Don't forget the number of EVs per LV feeder.

        Third: For each selected house with EV, randomly select an EV charging profile
            to add to the sym_load of that house.
        No profile can be used twice -> there will be enough profiles to cover all of sym_load.
