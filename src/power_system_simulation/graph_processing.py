...
class IDNotFoundError(Exception):
  def __init__(self, type):
    Exception.__init__(self, "IDNotFoundError: vertex pair does not exist in vertex_ids (if 0) or source_vertex_id does not exist in vertex_ids (if 1): T" + str(type))

class InputLengthDoesNotMatchError(Exception):
  def __init__(self, type, l_edge_vertex_id_pairs, l_edge_ids):
    Exception.__init__(self, "InputLengthDoesNotMatchError: The length of edge_enabled (if 0) or edge_vertex_id_pairs (if 1) (" + str(l_edge_vertex_id_pairs) + ") is different than edge_ids (" + str(l_edge_ids) + "): T" + str(type))

class IDNotUniqueError(Exception):
  def __init__(self, type):
    Exception.__init__(self, "IDNotUniqueError: There are non-unique vertices (if 0) or edges (if 1) in the graph: T" + str(type))
...
class GraphProcessor:
    ...
    def __init__(
        ...
        for x in edge_vertex_id_pairs:
          if not (x[0] in vertex_ids) or not (x[1] in vertex_ids):
            raise IDNotFoundError(0)
        # Check: source_vertex_id - is source vortex id valid?
        if not (source_vertex_id in vertex_ids):
